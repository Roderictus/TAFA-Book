...
P2012Secc %>%
  arrange(Lista_Nominal_Mun) %>%
  select(MUNICIPIO, Por_PAN, Por_PRI, Por_MORENA) %>%
  filter(MUNICIPIO %in% (PueMun2016 %>%
                           top_n(10, Lista_Nominal))$MUNICIPIO) %>%
  gather(key = MUNICIPIO, value = Partido) %>%
  setNames (.,c("Municipio", "Partido", "Porcentaje")) %>%
  ggplot(aes(x = Municipio, y = Porcentaje, fill = Partido)) + geom_boxplot() + 
