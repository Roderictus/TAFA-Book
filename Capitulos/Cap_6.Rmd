...
Realizamos un análisis de la situación económica de la población en el estado de Jalisco. La información relativa a ingresos por ocupación proviene de la Encuesta Nacional de Empleo (ENOE) para el último trimestre disponible a la elaboración de esta sección, 3er trimestre de 2017. La información a nivel vivienda (datos sobre jefatura de hogar, recibo de remesas) fueron obtenidas a partir de la encuesta Intercensal 2015 que es estadísticamente significativa a nivel Municipal. Los datos de distribución poblacional (piramide poblacional), fueron calculados utilizando los factores de expansión de la encuesta intercensal y coinciden con las estadísticas oficiales.

El análisis de ingreso aquí presentado es distinto a los oficialmente reportados, aunque se basa en los mismos datos. Los datos comunmente reportados se refieren a términos como población económicamente activa(PEA) y no activa (PNEA), ocupada, ocupada parcialmente, formal, informal, etc. dado que los porcentajes de estas variables son cálculados sobre sub-categorias (ej.formalidad es una proporción de PEA ocupada) no transmiten fácilmente un entendimiento sobre la situación económica de la población y requieren de un análisis que involucre el entendimiento de estas categorías. 

En el análisis que aquí se presenta se utilizan conceptos más básicos. La población que tiene ingresos positivos es aquella que reporta tener ingresos por ocupación mensuales mayores a cero. Una consideración es que la población que afirma no tener ingresos por ocupación no necesariamente se encuentra desocupada, puede encontrarse ocupada en actividades domésticas sin paga, negocios familiares, agricultura de subsistencia. Al considerar toda la población mayor de catorce años y sus ingresos por ocupación, consideramos aquella población que se encuentra estudiando y aquella que no puede trabajar por enfermedad, edad avanzada o cualquier otra causa. El utilizar ingresos por ocupación en lugar de las categorías usadas comúnmente sobre empleo permite entender mejor la situación de vulnerabilidad de la población respecto a sus ingresos, particularmente el contraste entre los ingresos por ocupación entre hombres y mujeres.
...
Presentamos una tabla con los ingresos promedio mensuales de los hogares en los principales municipios, desviación estandar, y promedio de ingreso de acuerdo al sexo del jefe del hogar

Este es un mapa con el ingreso promedio por jefatura de hogar
...
ViviendaIC %>% 
  ...
  summarise(Hogares = sum(FACTOR), 
            Ingreso_promedio_mensual =  weighted.mean((INGTRHOG)/3, w = FACTOR))
```
head(viviendaIC)


Ingreso promedio mensual del hogar de acuerdo a edad y sexo del jefe del hogar 
